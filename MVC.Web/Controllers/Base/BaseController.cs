    ...
    public class BaseController : Controller
    {
        ...
        protected override void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            try
            {
                ...
                try
                {
                    //Active menu check and open
                    var cacheItemKeymenus = "jApMenuBar" + Env.GetUserInfo("userid") + "Us" + Env.GetUserInfo("roleid");
                    var menus = (MenuPermission[])HttpRuntime.Cache.Get(cacheItemKeymenus);
                    if (menus != null)
                    {
                        var check = menus.FirstOrDefault(i => i.Menu_MenuId.MenuURL.ToLower() == controllerName);
                        if(check!=null)
                        {
                            ViewBag.MenuOpen = check.Menu_MenuId.Menu2.MenuText;
                        }
                    }
                }
                catch (Exception)
